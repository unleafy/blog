...
开始题意理解错了，以为是倒数第二位开始的连续的 $0$，后面也没有什么新的思路，就用 _python_ 打了个 $n \le 5000$ 的表，拿了 $50$ 分的部分分。
...
其中有一个 _Subtask 3_ 对问题的思考是很有帮助的。
...
首先我们考虑 _Subtask 3_ $n \le 5000$ 怎么做，很显然需要一个复杂度为 $\Theta(n^2)$ 的做法。考虑动态规划，定义状态 $dp_{i,j,0/1}$，表示左侧走了前 $i$ 队猫咪🐱，右侧走了前 $j$ 队猫咪，$0/1$ 表示上一次走的是左侧/右侧的猫咪。
